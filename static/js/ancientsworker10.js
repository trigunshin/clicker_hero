...
var BaseHSSouls = (Math.log(Heroes[33].cost / 20) / Math.log(1.07) + 1) / 2000;
...
function GetHeroLevelSouls(gold) {
  gold /= 26;
  if (gold < Heroes[33].cost) {
    return 0;
  }
  return Math.floor(BaseHSSouls + Math.log(0.07 * (gold - Heroes[33].cost) / Heroes[33].cost + 1) / Math.log(1.07) * 26 / 2000);
}
...
function LevelingPlan(argaiv, damageFactor) {
  ...
  //Find the next level of the hero that will attain the given (mined) dps
  ...
  // result plan steps
  this.plan = [];
  // cache next required level per hero
  this.bestList = [];
  var curDamage = 0;
  var curLevels = [];
  ...
  while (true) {
    ...
    //Add plan steps in batches of 25 levels up to the target
    while (curLevels[nextBest] < this.bestList[nextBest].level) {
